...
## Contexto Grupo BECM — Integraciones de Pago (CLAUDE.md v2.2.0 §10)

### Proveedores específicos
- **TAPI**: Tokenización de tarjetas, datos NUNCA tocan backend
- **STP (SPEI)**: Transferencias interbancarias en México, CLABE 18 dígitos
- **JAAK**: Verificación de identidad (KYC), validación INE/IFE
- **Twilio**: SMS OTP para MFA backup
- **FCM**: Push notifications

### Patrón de webhooks (§5.4)
- HMAC con timingSafeEqual, timestamp ±5min, nonce Redis SET NX
- Validar ANTES de encolar a RabbitMQ
- Retry exponencial: 5s, 10s, 20s → DLQ después de 3 intentos
- Controller dedicado por proveedor (TapiWebhookController, StpWebhookController)

### Idempotencia financiera
- Patrón atómico Redis SET NX (NUNCA GET→process→SET)
- TTL 24h para keys de idempotencia
- Cache de resultado para responses idempotentes

### Validadores mexicanos
- CLABE: 18 dígitos, validación módulo 10
- CURP: 18 caracteres, formato específico
- RFC: 12-13 caracteres con dígito verificador

### PCI DSS v4.0
- KEK/DEK con AES-256-GCM para datos sensibles
- LogSanitizingInterceptor: sanitizar PAN (Luhn), CVV, track data
- Solo almacenar: últimos 4 dígitos, BIN (primeros 6)

### Niveles KYC
- Nivel 0: $1,000 MXN | Nivel 1: $8,000 MXN | Nivel 2: $50,000 MXN | Nivel 3: Sin límite
- `@RequireKycLevel()` decorator en endpoints financieros
- Límites enforced en BACKEND (no solo frontend)

### Formato monetario
- `Intl.NumberFormat('es-MX')` → `$1,234.56 MXN`
- Zona horaria: America/Mexico_City
- CFDI 4.0 para facturación electrónica cuando aplique